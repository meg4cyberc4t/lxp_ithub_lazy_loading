...
class LazyLoadingRequest<T> {
  ...
  final Future<AbstractLazyLoadingModel<T>> Function(int page)
      loadingDataFromPage;
  ...
  Future<List<T>> loadingNextData() async {
    ...
    try {
      AbstractLazyLoadingModel model = await loadingDataFromPage(pageNumber);
      if (model is NewLazyLoadingModel) {
      ...
      } else if (model is LazyLoadingModel) {
        meta = model.meta;
